...
interoception_datasize = 300 * 2
fBase = 32
const = 1e-6


class Encoder_interoception(nn.Module):
    def __init__(self, latent_dim):
        super(Encoder_interoception, self).__init__()
        self.flatten = nn.Flatten()
        self.enc = nn.Sequential(
            nn.Linear(in_features=interoception_datasize, out_features=512),
            nn.ReLU(True),
...
class Decoder_interoception(nn.Module):
    def __init__(self, latent_dim):
        super(Decoder_interoception, self).__init__()
        self.dec = nn.Sequential(
            ...
            nn.ReLU(True),
            nn.Linear(in_features=512, out_features=interoception_datasize),
            nn.ReLU(True),
        )
...
class VAE(nn.Module):
    def __init__(self, latent_dim, device):
        ...
        self.encoder = Encoder_interoception(latent_dim)
        self.decoder = Decoder_interoception(latent_dim)
        self.device = device
...
def train(args, model, data_loader, optimizer, device='cuda', mu_prior=None, var_prior=None, save_epoch=0):
    model.train()
    loss_average = 0
    num_batch = 0
